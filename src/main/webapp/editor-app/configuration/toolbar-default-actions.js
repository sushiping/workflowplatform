...
KISBPM.TOOLBAR = {
    ACTIONS: {
        ...
        exportProcess: function (services) {
        	var modelMetaData = services.$scope.editor.getModelMetaData()
            window.open("module/export?modelId="+modelMetaData.modelId);
        },
...
//                window.location.href = "./";
        ...
        $scope.deployAndClose = function () {
            $scope.save(function () {
            	var modelId = $scope.editor.getModelMetaData().modelId;
            	$http({
        			url : '/workflow/process/deployProcess.action?modelId='+modelId,
        			method : 'POST', // 默认值: "GET"。请求方式 ("POST" 或 "GET")， 默认为 "GET"
        			 headers: {
        		...
        		.error(function(data, status, headers, config){
        			alert("系统请求失败！（有可能是流程节点为空或只存在开始、结束节点导致，请修改后重新发布）");
        		});
            });
        };
